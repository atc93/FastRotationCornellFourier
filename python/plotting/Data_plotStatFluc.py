...
plt.suptitle('$\mathregular{<x_{e}>}=' + '{0:.2f}'.format(mean_xe) + '$ +- ' + '{0:.2f}'.format(std_xe) + ' mm')
plt.hist(xe, bins=30)
plt.savefig('plots/eps/' + tag + '_xe.eps', format='eps')

plt.figure(3)
plt.xlabel('$\mathregular{\sigma}$ [mm]')
plt.suptitle('$\mathregular{<\sigma>}=' + '{0:.2f}'.format(mean_width) + '$ +- ' + '{0:.2f}'.format(std_width) + ' mm')
plt.hist(width, bins=30)
plt.savefig('plots/eps/' + tag + '_width.eps', format='eps')

plt.figure(4)
plt.xlabel('$\mathregular{t_{0}}$ [ns]')
plt.suptitle('$\mathregular{<t_{0}>}=' + '{0:.2f}'.format(mean_t0) + '$ +- ' + '{0:.2f}'.format(std_t0) + ' ns')
plt.hist(t0, bins=30)
